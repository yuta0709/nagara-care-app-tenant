...
import type { Route } from "./+types/_authenticated.residents.$uid.elimination-records.new";
...
import { createEliminationRecord, getResident } from "~/api/nagaraCareAPI";
...
export async function clientAction({ request, params }: Route.ActionArgs) {
  ...
  const response = await createEliminationRecord(uid, data);
  return redirect(`/residents/${uid}/elimination-records/${response.uid}`);
}
...
export default function NewEliminationRecordPage({
  ...
  const handleBack = () => {
    navigate(`/residents/${resident.uid}/elimination-records`);
  };
